...
class Model {
    ...
    boolean placeable(Piece piece, int row, int col) {
        ...
        if (row + piece.height() > height() || col + piece.width() > width()) {
            return false;
        }
        for (int r = 0; r < piece.height(); r += 1) {
            for (int c = 0; c < piece.width(); c += 1) {
                if (piece.get(r, c) && _cells[row + r][col + c]) {
                    return false;
                }
            }
        }
    ...
    private int scoreClearedLines(int nrows, int ncols) {
        return (_streakLength * ((nrows * width() + ncols * height())));
    }
